...
stock_assess_areas_rs <- ROracle::dbSendQuery(conn = garfo.connection,
                                          statement = stringr::str_c("SELECT DISTINCT * FROM cams_garfo.cfg_stock_assessment_areas WHERE ITIS_TSN IN", nefmc_species_itis, "AND ASSESSMENT_TYPE = 'STANDARD' AND ASSESSMENT_STOCK_AREA NOT IN 'OUTSIDE'", sep = " "))

# save the database query into an R object.
# statarea_stock <- DBI::dbFetch(statarea_stock_rs)
stock_assess_areas <- DBI::dbFetch(stock_assess_areas_rs)
...
nefmc_species <- nefmc_species |>
  ...
  dplyr::left_join(stock_assess_areas, by = c("ITIS_TSN")) |>
  # add columns where:
  dplyr::mutate(
    # the beginning of the fishing year is dependent on the FMP
    BEGIN_FY_MONTH = dplyr::case_when(
    ...
    STOCK_NAME = dplyr::case_when(
                  ...
                  SPECIES_NAME == "Atlantic cod" & AREA %in% c(533:534, 537:539, 611:640) ~ stringr::str_c("SNE", SPECIES_NAME, sep = " "),
                  # when rows with Atlantic cod also have areas in this list, the stock name is WGOM Atlantic cod
                  SPECIES_NAME == "Atlantic cod" & AREA %in% c(513:515,521,526,541) ~ stringr::str_c("WGOM", SPECIES_NAME, sep = " "),
                  # when rows with Atlantic cod also have areas in this list, the stock name is GBK Atlantic cod
                  SPECIES_NAME == "Atlantic cod" & AREA %in% c(464,522,525,542:562) ~ stringr::str_c("GBK", SPECIES_NAME, sep = " "),
                  # when rows with Atlantic cod also have areas in this list, the stock name is GB Atlantic cod
                  # SPECIES_NAME == "Atlantic cod" & AREA %in% c(551:552,561:562) ~ stringr::str_c("EGB", SPECIES_NAME, sep = " "),
                  # when rows with Atlantic cod also have areas in this list, the stock name is EGOM Atlantic cod
                  SPECIES_NAME == "Atlantic cod" & AREA %in% c(465:512) ~ stringr::str_c("EGOM", SPECIES_NAME, sep = " "),
                  # when rows with Haddock also have the GOM region, the stock name is GOM Haddock
                  # SPECIES_NAME == "Haddock" & SPECIES_ESTIMATION_REGION %in% c("GOM", "EGB") ~ stringr::str_c(SPECIES_ESTIMATION_REGION, SPECIES_NAME, sep = " "),
                  # when rows with Haddock also have regions in this list, the stock name is WGB Haddock
                  # SPECIES_NAME == "Haddock" & SPECIES_ESTIMATION_REGION %in% c("WGB_SNE", "MA") ~ stringr::str_c("WGB", SPECIES_NAME, sep = " "),
                  # when rows with Windowpane flounder also have the SNEMA in the assessment stock area column, the stock name is Southern Windowpane flounder
                  SPECIES_NAME == "Windowpane flounder" & ASSESSMENT_STOCK_AREA == "SNEMA" ~ stringr::str_c("Southern", SPECIES_NAME, sep = " "),
                  # when rows with Windowpane flounder also have GBGOM in the assessment stock area column, the stock name is Northern Windowpane flounder
                  SPECIES_NAME == "Windowpane flounder" & ASSESSMENT_STOCK_AREA == "GBGOM" ~ stringr::str_c("Northern", SPECIES_NAME, sep = " "),
                  # when rows with Monkfish also have the SOUTH in the assessment stock area column, the stock name is Southern Monkfish
                  SPECIES_NAME == "Monkfish" & ASSESSMENT_STOCK_AREA == "SOUTH" ~ stringr::str_c("Southern", SPECIES_NAME, sep = " "),
                  # when rows with Monkfish also have the NORTH in the assessment stock area column, the stock name is Northern Monkfish
                  SPECIES_NAME == "Monkfish" & ASSESSMENT_STOCK_AREA == "NORTH" ~ stringr::str_c("Northern", SPECIES_NAME, sep = " "),
                  # when rows with Red hake also have SOUTH in the assessment stock area column, the stock name is Southern red hake
                  SPECIES_NAME == "Red hake" & ASSESSMENT_STOCK_AREA == "SOUTH" ~ stringr::str_c("Southern", SPECIES_NAME, sep = " "),
                  # when rows with Red hake also have NORTH in the assessment stock area column, the stock name is Northern red hake
                  SPECIES_NAME == "Red hake" & ASSESSMENT_STOCK_AREA == "NORTH" ~ stringr::str_c("Northern", SPECIES_NAME, sep = " "),
                  # when rows with Silver hake also have NORTH in the assessment stock area column, the stock name is Northern silver hake
                  SPECIES_NAME == "Silver hake" & ASSESSMENT_STOCK_AREA == "NORTH" ~ stringr::str_c("Northern", SPECIES_NAME, sep = " "),
                  # when rows with Silver hake also have SOUTH in the assessment stock area column, the stock name is Southern whiting and also includes Offshore hake
                  SPECIES_NAME == "Silver hake" & ASSESSMENT_STOCK_AREA == "SOUTH" ~ "Southern whiting",
                  # Offshore is included as a part of the Southern whiting stock with Southern silver hake, the stock name is Southern whiting regardless of the assessment stock area column values
                  SPECIES_NAME == "Offshore hake" ~ "Southern whiting",
                  # when rows with Yellowtail flounder also have regions in this list, the stock name is SNEMA Yellowtail flounder
                  # SPECIES_NAME == "Yellowtail flounder" & SPECIES_ESTIMATION_REGION %in% c("SNE", "MA") ~ stringr::str_c("SNEMA", SPECIES_NAME, sep = " "),
                  ...
                  # SPECIES_NAME == "Yellowtail flounder" & SPECIES_ESTIMATION_REGION %in% c("GB", "CCGOM") ~ stringr::str_c(SPECIES_ESTIMATION_REGION, SPECIES_NAME, sep = " "),
                  # for rows with Winter flounder, the stock name is the combination of the region and Winter flounder flounder
                  # SPECIES_NAME == "Winter flounder"  ~ stringr::str_c(SPECIES_ESTIMATION_REGION, SPECIES_NAME, sep = " "),
                  # for rows with species in this list, the stock name is the combination of the region and the species
                  SPECIES_NAME %in% c("Haddock", "Yellowtail flounder", "Winter flounder") ~ stringr::str_c(ASSESSMENT_STOCK_AREA, SPECIES_NAME, sep = " "),
